...
class AlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        try {
            ...
            FileLog.e(
                context,
                "闹钟：续排失败！${t.javaClass.simpleName}:${t.message}"
            )
            // ⚠️ 不 return，本轮还能继续做事
        }
...
if (cellSnap == null) {
    ...
    cellSnap.cells.forEachIndexed { index, c ->
        ...
        try {
            ...
            FileLog.w(
                context,
                "闹钟：前台服务被系统拒绝（后台限制）：${e.message}"
            )
            // 👉 没关系，你已经有“基站兜底”了
        } catch (t: Throwable) {
            FileLog.e(
                context,
                "闹钟：启动前台服务异常 ${t.javaClass.simpleName}:${t.message}"
            )
        }
        ...
        try {
            ...
            FileLog.w(
                context,
                "闹钟：确保周期保险失败（不影响主链）${t.javaClass.simpleName}:${t.message}"
            )
        }
